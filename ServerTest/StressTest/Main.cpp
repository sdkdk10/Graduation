...
#include "../../SlashServer/SlashServer/MiniDump.h"
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glRasterPos2f(-0.55, 0.38);
	glPrint("STRESS TEST [%d]", cuser);	// Print GL Text To The Screen

	int size = 0;
	float *points = nullptr;
	TestManager::GET_INSTANCE()->GetPointCloud(&size, &points);
	cuser = size;
	...
	for (int i = 0; i < size; i++)
	{
		...
		x = (-0.8 + points[i * 2] / 550.0);
		y = (0.60 - points[i * 2 + 1] / 550.0);
		z = -1.0f;
		glVertex3f(x, y, z);
	}
	glEnd();
	...
	int			nCmdShow)			// Window Show State
{
	...
	TestManager::GET_INSTANCE()->StartTest();
	...
	TestManager::GET_INSTANCE()->JoinThreads();
