...
HRESULT CSkill_Billboard::Initialize()
{
	...
	size_t needed = ::mbstowcs(NULL, &m_strTexName[0], m_strTexName.length());
	...
	Mat->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.f);
	Mat->FresnelR0 = XMFLOAT3(0.05f, 0.05f, 0.05f);
	Mat->Roughness = 0.3f;

	/* CB(World,TextureTranform...) Build */

	m_pTransCom->Scaling(1.f, 1.f, 1.f);
	m_pTransCom->Translate(0.f, 0.f, 0.f);
...
bool CSkill_Billboard::Update(const GameTimer & gt)
{
	...
	XMMATRIX matRotY = XMMatrixRotationX(m_pTransCom->GetRotation().y);
	XMMATRIX matRotZ = XMMatrixRotationX(m_pTransCom->GetRotation().z);
