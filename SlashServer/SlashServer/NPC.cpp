...
void NPC::Initialize()
{
	...
	//lua_getglobal(L_, "LoadMonsterData");
	//lua_pcall(L_, 0, 5, 0);
	//world_._41 = static_cast<float>(lua_tonumber(L_, -5));
	//world_._42 = 0;
	//world_._43 = static_cast<float>(lua_tonumber(L_, -4));
	//hp_ = (int)lua_tonumber(L_, -3);
	//dmg_ = (int)lua_tonumber(L_, -2);
	//npcType_ = (int)lua_tonumber(L_, -1);
	//lua_pop(L_, 5);
