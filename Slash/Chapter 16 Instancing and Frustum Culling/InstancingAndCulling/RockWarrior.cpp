...
RockWarrior::RockWarrior(Microsoft::WRL::ComPtr<ID3D12Device> d3dDevice, ComPtr<ID3D12DescriptorHeap> &srv, UINT srvSize)
...
HRESULT RockWarrior::Initialize()
{
	m_pMesh = dynamic_cast<DynamicMeshSingle*>(CComponent_Manager::GetInstance()->Clone_Component(L"Com_Mesh_RockWarrior"));
	if (nullptr == m_pMesh)
		return E_FAIL;



	Texture* tex = CTexture_Manager::GetInstance()->Find_Texture("RockWarriorTex", CTexture_Manager::TEX_DEFAULT_2D);
	if (nullptr == tex)
		return E_FAIL;
...
void RockWarrior::Animate(const GameTimer & gt)
{
	AnimStateMachine->AnimationStateUpdate(gt);

}

void RockWarrior::SaveSlidingVector(CGameObject * pobj, CGameObject * pCollobj)
{
...
RockWarrior * RockWarrior::Create(Microsoft::WRL::ComPtr<ID3D12Device> d3dDevice, ComPtr<ID3D12DescriptorHeap>& srv, UINT srvSize)
{
	RockWarrior* pInstance = new RockWarrior(d3dDevice, srv, srvSize);
...
AnimateStateMachine_RockWarrior::~AnimateStateMachine_RockWarrior()
{
}
...
AnimateStateMachine_RockWarrior * AnimateStateMachine_RockWarrior::Create(CGameObject* pObj, wchar_t * pMachineName, int SoundFrame[State::STATE_END], int EffectFrame[State::STATE_END])
{
	AnimateStateMachine_RockWarrior* pInstance = new AnimateStateMachine_RockWarrior(pObj, pMachineName, SoundFrame, EffectFrame);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX(L"AnimateStateMachine_RockWarrior Created Failed");
		Safe_Release(pInstance);
	}
...
void AnimateStateMachine_RockWarrior::Free()
{
}