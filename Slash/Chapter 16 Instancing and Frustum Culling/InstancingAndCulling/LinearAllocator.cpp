...
size_t LinearAllocatorPage::Suballocate(_In_ size_t size, _In_ size_t alignment)
{
    ...
    if (offset + size > mSize)
        throw exception("Out of free memory in page suballoc");
#endif
    mOffset = offset + size;
    return offset;
}
...
#ifdef _DEBUG
    if (size > m_increment)
        throw out_of_range("Size must be less or equal to the allocator's increment");
    if (alignment > m_increment)
        throw out_of_range("Alignment must be less or equal to the allocator's increment");
    if (size == 0)
        throw exception("Cannot honor zero size allocation request.");
#endif
...
void LinearAllocator::FenceCommittedPages(_In_ ID3D12CommandQueue* commandQueue)
{
    ...
    for (LinearAllocatorPage* page = m_usedPages; page != nullptr; page = nextPage)
    {
        ...
        if (page->RefCount() == 1)
        {
            // Signal the fence
            numReady++;
			if (FAILED((commandQueue->Signal(page->mFence.Get(), ++page->mPendingFence))))
			{
				MSG_BOX(L"LinearAllocate::commandQueue->Signal(page->mFence.Get(), ++page->mPendingFence) Failed!!");
			}
...
LinearAllocatorPage* LinearAllocator::GetNewPage()
{
	...
	if (FAILED((spResource->Map(0, nullptr, &pMemory))))
	{
		MSG_BOX(L"LinearAllocator::spResource->Map(0, nullptr, &pMemory) FAILED!!");
	}
    ...
    if (FAILED(m_device->CreateFence(
        ...
        IID_PPV_ARGS(spFence.ReleaseAndGetAddressOf()))))
    {
        return nullptr;
    }
