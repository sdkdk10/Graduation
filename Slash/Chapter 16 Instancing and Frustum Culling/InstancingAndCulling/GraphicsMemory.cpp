...
namespace
{
    ...
    inline size_t GetPageSizeFromPoolIndex(size_t x)
    {
        ...
        return  max<size_t>(MinPageSize, size_t(1) << (x + AllocatorIndexShift));
    }
    ...
    public:
        ...
        {
            if (!device)
                throw  invalid_argument("Invalid device parameter");

            for (size_t i = 0; i < mPools.size(); ++i)
            {
                size_t pageSize = GetPageSizeFromPoolIndex(i);
                mPools[i] =  make_unique<LinearAllocator>(
                    mDevice.Get(),
                    pageSize);
            }
        }
    ...
    private:
        ComPtr<ID3D12Device> mDevice;
         array< unique_ptr<LinearAllocator>, AllocatorPoolCount> mPools;
         mutex mMutex;
    };
} // anonymous namespace
...
public:
    ...
    {
        mDeviceAllocator =  make_unique<DeviceAllocator>(device);

    #if !defined(_XBOX_ONE) || !defined(_TITLE)
        if (s_graphicsMemory.find(device) != s_graphicsMemory.cend())
        {
            throw  exception("GraphicsMemory is a per-device singleton");
        }
...
#else
    static map<ID3D12Device*, GraphicsMemory::Impl*> s_graphicsMemory;
#endif

private:
     unique_ptr<DeviceAllocator> mDeviceAllocator;
};

#if defined(_XBOX_ONE) && defined(_TITLE)
GraphicsMemory::Impl* GraphicsMemory::Impl::s_graphicsMemory = nullptr;
#else
 map<ID3D12Device*, GraphicsMemory::Impl*> GraphicsMemory::Impl::s_graphicsMemory;
#endif
...
GraphicsMemory& GraphicsMemory::Get(_In_opt_ ID3D12Device*)
{
    if (!Impl::s_graphicsMemory || !Impl::s_graphicsMemory->mOwner)
        throw  exception("GraphicsMemory singleton not created");
...
GraphicsMemory& GraphicsMemory::Get(_In_opt_ ID3D12Device* device)
{
    if (Impl::s_graphicsMemory.empty())
     ...
     map<ID3D12Device*, GraphicsMemory::Impl*>::const_iterator it;
    if (!device)
    {
    ...
    if (it == Impl::s_graphicsMemory.cend() || !it->second->mOwner)
        throw  exception("GraphicsMemory per-device singleton not created");
...
SharedGraphicsResource::SharedGraphicsResource(GraphicsResource&& resource)
    : mSharedResource( make_shared<GraphicsResource>( move(resource)))
{
}

SharedGraphicsResource::SharedGraphicsResource(SharedGraphicsResource&& resource) noexcept
    : mSharedResource( move(resource.mSharedResource))
{
}
