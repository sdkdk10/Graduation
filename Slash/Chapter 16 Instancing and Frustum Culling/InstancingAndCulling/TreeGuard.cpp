...
bool TreeGuard::Update(const GameTimer & gt)
{
...
HRESULT TreeGuard::Initialize()
{
	m_pMesh = dynamic_cast<DynamicMeshSingle*>(CComponent_Manager::GetInstance()->Clone_Component(L"Com_Mesh_TreeGuard"));
	if (nullptr == m_pMesh)
		return E_FAIL;



	Texture* tex = CTexture_Manager::GetInstance()->Find_Texture("TreeGuardTex", CTexture_Manager::TEX_DEFAULT_2D);
	if (nullptr == tex)
		return E_FAIL;
...
void TreeGuard::Animate(const GameTimer & gt)
{
	AnimStateMachine->AnimationStateUpdate(gt);

}

void TreeGuard::SaveSlidingVector(CGameObject * pobj, CGameObject * pCollobj)
{
...
TreeGuard * TreeGuard::Create(Microsoft::WRL::ComPtr<ID3D12Device> d3dDevice, ComPtr<ID3D12DescriptorHeap>& srv, UINT srvSize)
{
	TreeGuard* pInstance = new TreeGuard(d3dDevice, srv, srvSize);
