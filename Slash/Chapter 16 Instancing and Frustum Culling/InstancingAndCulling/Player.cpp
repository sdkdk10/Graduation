...
// > ===========================================================
	...
	AnimStateMachine->AnimationStateUpdate(gt); //¾Ö´Ï¸ÞÀÌ¼Ç »óÅÂ ¼³Á¤ÇØÁÖ´Â ÇÔ¼ö
	...
	// ¹ÞÀ» ¶§ °»½ÅÇØ¾ßÇÔ // ¿ùµå Çà·Ä·Î..? // ¼­¹öÀÛ¾÷ // È¸Àü ¹ÞÀ»¶§¶û // À§Ä¡ ¹ÞÀ»¶§ // ¾Æµµ°¡ ±×³É ´ëÃæ ÇÏÀÚ°íÇÔ
...
{
	//return;
	AnimStateMachine->SetTimerTrueFalse(); //¾î¶² ¾Ö´Ï¸ÞÀÌ¼ÇÀ» µ¿ÀÛ ½ÃÄÑÁÖ´Â Áö 
...
HRESULT Player::Initialize()
{
	...
	Texture* tex = CTexture_Manager::GetInstance()->Find_Texture(strTexName, CTexture_Manager::TEX_DEFAULT_2D);			// ÀÌ·±½ÄÀ¸·Î °¡Á®¿È
	if (tex == nullptr)
		return E_FAIL;
	...
	Mat->DiffuseSrvHeapIndex = tex->Num;			// ÅØ½ºÃÄ Èü¿¡ ÀúÀå¤§¿Ü³ª ¼ö¾Æ¾Æ¤·¾Æ¤¿ ¾ÆÇÏ ÁöÈñ¾ß±×·³ ÀÌ°ÅÀÌ·¸°Ô¹Ù²ã¾ßÇÔ 
...
>>>>>>> parent of 19ae9e7... ê¶ê·¹ê¸° ì¶”ê°€ , ìš© ëª¬ìŠ¤í„° ì¶”ê°€
		...
		if (!m_IsEffectPlay[AnimateStateMachine::STATE_ATTACK1] && m_fAnimationKeyFrameIndex_Attack1 > m_EffectFrame[AnimateStateMachine::STATE_ATTACK1])
		{
			...
			// > ½ºÅ³³Ö¾îÁÖ±â
			//CEffect_Manager::GetInstance()->Play_SkillEffect("½ºÅ³ÀÌ¸§");
			//cout << "½ºÅ³!" << endl;
	...
	if (bTimerAttack3 == true)
	{
		...
		if (!m_IsSoundPlay[AnimateStateMachine::STATE_ATTACK3] && m_fAnimationKeyFrameIndex_Attack3 > m_SoundFrame[AnimateStateMachine::STATE_ATTACK3])
		{
			...
			//CManagement::GetInstance()->GetSound()->PlayEffect(m_pMachineName, m_pStateName[AnimateStateMachine::STATE_ATTACK3]);		// > ¸ðµç »ç¿îµå°¡ µé¾î°¬À»¶§ ÀÌ·¸°Ô ¹Ù²ã¾ßÇÔ!
			CManagement::GetInstance()->GetSound()->PlayEffect(L"Sound", L"Attack");
		}
